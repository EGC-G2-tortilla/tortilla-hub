...
def test_client(test_client):
    with test_client.application.app_context():
        ...
        db.session.add_all([ds_metrics1, ds_metrics2, ds_metrics3, ds_metrics4, ds_metrics5])
        db.session.commit()

        ds_meta1 = DSMetaData(title="Dataset 1", dataset_doi="doi1", ds_metrics_id=ds_metrics1.id,
                              description="description1...", publication_type=PublicationType.BOOK)
        ds_meta2 = DSMetaData(title="Dataset 2", dataset_doi="doi2", ds_metrics_id=ds_metrics2.id,
                              description="description2...", publication_type=PublicationType.BOOK)
        ds_meta3 = DSMetaData(title="Dataset 3", dataset_doi="doi3", ds_metrics_id=ds_metrics3.id,
                              description="description3...", publication_type=PublicationType.REPORT)
        ds_meta4 = DSMetaData(title="Dataset 4", dataset_doi="doi4", ds_metrics_id=ds_metrics4.id,
                              description="description4...", publication_type=PublicationType.BOOK)
        ds_meta5 = DSMetaData(title="Dataset 5", dataset_doi="doi5", ds_metrics_id=ds_metrics5.id,
                              description="description5...", publication_type=PublicationType.BOOK)
        ...
        download_record1 = DSDownloadRecord(dataset_id=dataset1.id, download_cookie="cookie1")
        download_record2 = DSDownloadRecord(dataset_id=dataset2.id, download_cookie="cookie2")
        download_record3 = DSDownloadRecord(dataset_id=dataset3.id, download_cookie="cookie3")
        download_record4 = DSDownloadRecord(dataset_id=dataset4.id, download_cookie="cookie4")
        download_record5 = DSDownloadRecord(dataset_id=dataset5.id, download_cookie="cookie5")
        download_record6 = DSDownloadRecord(dataset_id=dataset1.id, download_cookie="cookie6")
        download_record7 = DSDownloadRecord(dataset_id=dataset1.id, download_cookie="cookie7")
        download_record8 = DSDownloadRecord(dataset_id=dataset2.id, download_cookie="cookie8")
        download_record9 = DSDownloadRecord(dataset_id=dataset3.id, download_cookie="cookie9")
        download_record10 = DSDownloadRecord(dataset_id=dataset4.id, download_cookie="cookie10")
        db.session.add_all([download_record1, download_record2, download_record3, download_record4,
                            download_record5, download_record6, download_record7, download_record8,
                            download_record9, download_record10])
        db.session.commit()
...
def test_most_popular_authors(test_client):
    with test_client.application.app_context():
        ...
        assert all(author.name in ["Author 1", "Author 2", "Author 3", "Author 4"] for author in popular_authors)
...
def test_most_downloaded_negative(test_client):
    with test_client.application.app_context():
        most_downloaded_datasets = dataset_repo.most_downloaded()
        assert not any(dataset.ds_meta_data.title == "Dataset 5" for dataset in most_downloaded_datasets)
...
def test_latest_synchronized_negative(test_client):
    with test_client.application.app_context():
        latest_datasets = dataset_repo.latest_synchronized()
        assert not any(dataset.ds_meta_data.title == "Dataset 1" for dataset in latest_datasets if dataset.id < 0)
