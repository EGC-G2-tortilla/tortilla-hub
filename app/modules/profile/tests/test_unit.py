...
def test_view_profile_unauthenticated(test_client):
    ...
    other_profile = UserProfile(user_id=other_user.id, name="Unauth", surname="User")
    db.session.add(other_profile)
    db.session.commit()

    response = test_client.get(f"/profile/summary/{other_user.id}")
    assert response.status_code == 200, "No problem with unauthenticated user profile."
...
def test_view_profile_sensitive_data_not_exposed(test_client):
    ...
    other_user = User(email="sensitive_user@example.com", password="password123")
    db.session.add(other_user)
    db.session.commit()

    other_profile = UserProfile(user_id=other_user.id, name="Sensitive", surname="Data")
    db.session.add(other_profile)
    db.session.commit()
    ...
    # Verificar que información sensible como correo o contraseña no está en la respuesta
